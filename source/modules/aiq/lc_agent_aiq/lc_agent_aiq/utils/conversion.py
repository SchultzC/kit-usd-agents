...
def convert_langchain_to_aiq_messages(lc_messages: List[BaseMessage]) -> AIQChatRequest:
    ...
    to the AIQ message format required for AIQ chat requests. It properly handles
    ...
    for openai_msg in openai_messages:
        ...
        if isinstance(content, list):
                ...
                if isinstance(item, dict):
                    if item.get("type") == "text":
                        ...
                        if isinstance(image_url_data, dict):
                            url_str = image_url_data.get("url", "http://default.com")
                        else:
                        ...
                        if url_str.startswith("data:image"):
                            ...
                            image_content = ImageContent(
                                ...
                                image_url=ImageData(url=HttpUrl("http://image_data"), data=url_str)
                            )
                            aiq_content.append(image_content)
                        elif len(url_str) < 2083 and url_str.startswith(("http://", "https://")):
            ...
            aiq_messages.append(Message(content=aiq_content, role=openai_msg["role"]))
        else:
            # Simple string content
            aiq_messages.append(Message(content=str(content), role=openai_msg["role"]))
